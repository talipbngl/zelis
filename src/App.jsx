...
export default function App() {
  ...
  // ✅ FIX: step overlay artık hedef index’i saklıyor
...
useEffect(() => {
  ...
  // container'a göre boyut (mobilde de düzgün)
  const setupSize = () => {
    ...
    // Kaplama: biraz daha "kaliteli" doku
  ...
  // Resize olursa tekrar kur
  const ro = new ResizeObserver(() => {
    setupSize();
  });
  if (canvas.parentElement) ro.observe(canvas.parentElement);

  // Brush: mobilde daha kalın
  const getBrushRadius = () => {
    const w = canvas.getBoundingClientRect().width;
    // mobil: 26-34, desktop: 18-26
  ...
  // performans için küçük ölçüm canvas'ı
  ...
  const scratchLine = (a, b, r) => {
    // çizgi halinde kazı: boşluk kalmasın
    ctx.beginPath();
    ctx.lineCap = "round";
    ctx.lineJoin = "round";
    ctx.lineWidth = r * 2;
  ...
  // daha hafif ölçüm: downsample alıp alpha say
  let lastMeasureTs = 0;
  const measure = () => {
    if (!sctx) return;

    const now = performance.now();
    if (now - lastMeasureTs < 180) return; // ölçümü çok sık yapma
    lastMeasureTs = now;
    ...
    // Canvas CSS boyutuna göre sample al
    sctx.clearRect(0, 0, sampleW, sampleH);
    // drawImage: canvas'ı sample boyuta indir
    sctx.drawImage(canvas, 0, 0, Math.floor(w), Math.floor(h), 0, 0, sampleW, sampleH);

    const data = sctx.getImageData(0, 0, sampleW, sampleH).data;

    // destination-out sonrası "şeffaf" yerler alpha düşük olur
    let cleared = 0;
    ...
    const ratio = cleared / total; // 0..1
    const pct = Math.round(ratio * 100);
    setScratchProgress(pct);

    // Eşik: %38 (ne çok kolay ne çok zor)
    if (ratio >= 0.38) {
  ...
  Kazıma: {scratchProgress}%
  {!scratchCanReveal ? " — biraz daha" : " — tamam"}
</div>
...
<div className="navRow reveal lift" style={{ "--d": "720ms", marginTop: 14 }}>
  <button
    ...
    type="button"
    onClick={() => {
      // çok uğraşana kolaylık: mesajı aç ama yine de 'Devam' butonu senin akışınla aynı kalsın
      setScratchCanReveal(true);
      setScratchReady(true);
      const msg = document.getElementById("scratchMsg");
      msg?.classList.add("scratchReveal");
  ...
  >
    Devam
      ...
      {/* STEP OVERLAY ✅ FIX: Devam basınca pendingIndex'e geç */}
...
function FlowHead({ p }) {
  ...
  const wrapIcon = (txt) => {
    ...
    if (!txt.includes(" ")) {
      ...
      if (looksLikeIcon) return <span className="iconEmoji solo" aria-hidden="true">{txt}</span>;
      return txt;
    }
    ...
    return (
      <>
        <span className="iconEmoji" aria-hidden="true">{first}</span>
